...
def read_conf(filename):
    with open(filename) as f:
        confobj = yaml.load(f, yaml.SafeLoader)
    return confobj
...
def get_keystone_client(api_version):
    if api_version == 3:
        ...
        auth = v3.Password(**ksc_vars)
        sess = session.Session(auth=auth)
        keystone = keystone_v3_client.Client(session=sess)
    else:
        ksc_vars = dict(
            username=os.environ['OS_USERNAME'],
            password=os.environ['OS_PASSWORD'],
...
class StatusExchange:
    ...
    def _setup_connection(self):
        ...
        try:
            ...
            ssl_opts = pika.SSLOptions(context, host)
            creds = pika.PlainCredentials(id_conf['rabbit_userid'],
                                                id_conf['rabbit_password'])

            params = pika.ConnectionParameters(host=host,
                                        port=id_conf['rabbit_port'],
                                        virtual_host=id_conf['rabbit_virtual_host'],
                                        credentials=creds,
